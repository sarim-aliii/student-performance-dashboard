...
export default function App() {
  ...
  const uniqueClasses = useMemo(() => {
    return [...new Set(records.map(r => r.class))].sort();
  }, [records]);
...
Status: ${record.marks >= 40 ? 'Pass' : 'Fail'}`;
  ...
  const processUpload = () => {
    if (!fileToUpload || !user || !uploadClassName.trim()) {
        ...
        try {
          const promises = results.data.map((row: any) => {
            const record: StudentRecord = {
              ...
              class: uploadClassName.trim(), // <-- Injecting the assigned class here!
          ...
          toast.success(`Successfully uploaded ${promises.length} records for Class ${uploadClassName}.`);
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 gsap-header-el">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <Dialog open={isUploadModalOpen} onOpenChange={(open) => {
                      ...
                      if (!open) { setFileToUpload(null); setUploadClassName(''); }
                    }}>
                      <DialogTrigger asChild>
                        <Button className="bg-indigo-600 text-white hover:bg-indigo-700 hover:shadow-md flex items-center gap-2 px-5 py-6 rounded-xl font-semibold transition-all shadow-sm">
                      ...
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Upload Class Records</DialogTitle>
                        </DialogHeader>
                        <div className="space-y-4 mt-4">
                          <div className="space-y-2">
                            ...
                            <p className="text-xs text-slate-500">Select an existing class or type a new one to create it.</p>
                          </div>
                          ...
                          <Button
                            ...
                            disabled={uploading || !fileToUpload || !uploadClassName.trim()}
                            className="w-full bg-indigo-600 hover:bg-indigo-700 py-6"
                          >
                            {uploading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Upload className="w-4 h-4 mr-2" />}
                            {uploading ? 'Processing Data...' : 'Upload & Assign to Class'}
                          </Button>
                        </div>
                      </DialogContent>
                    </Dialog>
